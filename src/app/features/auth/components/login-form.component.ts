    ...
    <div class="animate-fade-in-up">
      <div class="mb-8">
        <h3 class="text-3xl font-black text-[#0a1a0f] mb-2 flex items-center gap-3">
          <span class="material-symbols-outlined text-[#10b981] text-4xl">account_circle</span>
          Connexion
        </h3>
        <p class="text-gray-500 font-medium text-sm md:text-base">Saisissez vos identifiants pour participer aux jeux</p>
      </div>
      ...
      <form [formGroup]="formGroup" class="flex flex-col gap-5">
        ...
        <div>
          ...
          <div class="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:border-[#10b981] focus-within:ring-2 focus-within:ring-[#10b981]/20 transition-all bg-gray-50">
            <input tabindex="1" type="text" #email autocomplete="username" placeholder="prenom.nom"
              formControlName="email" (input)="onEmailInput.emit('login')"
        ...
        <div>
          ...
          <div class="relative flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:border-[#10b981] focus-within:ring-2 focus-within:ring-[#10b981]/20 transition-all bg-gray-50">
            <input tabindex="2" type="{{ passType }}" #pass placeholder="password" autocomplete="current-password"
              formControlName="pass" (keydown.enter)="submitLogin.emit({email: email.value + '@infomil.mu', pass: pass.value})"
          ...
          @if (formGroup.get('pass')?.invalid && formGroup.get('pass')?.touched) {
            <span class="text-red-500 text-xs font-semibold mt-1 block">Mot de passe invalide</span>
          }
        </div>

        @if (otherError) {
          <div class="bg-red-50 text-red-600 p-3 rounded-lg text-sm font-medium border border-red-100 mt-2 flex items-center gap-2">
        ...
        <div class="flex justify-end">
          <button tabindex="5" type="button"
            ...
            (click)="toggleForgotPassword.emit()">Mot de passe oublié ?</button>
        </div>

        <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span class="text-sm text-gray-500 font-medium">
            Pas de compte ?
            <button tabindex="4" type="button" class="font-bold text-[#0a1a0f] hover:text-[#10b981] transition-all duration-300 inline-block transform hover:scale-[1.03] active:scale-[0.97]"
              (click)="toggleRegister.emit()">S'inscrire</button>
          </span>
          <button tabindex="3" type="button"
            ...
            (click)="submitLogin.emit({email: email.value + '@infomil.mu', pass: pass.value})" [disabled]="!formGroup.valid">
            Connexion
          </button>
        </div>
      </form>
...
export class LoginFormInterfaceComponent {
  @Input() formGroup!: FormGroup;
  @Input() otherError: string = '';
  ...
  @Output() submitLogin = new EventEmitter<{ email: string; pass: string }>();
}
